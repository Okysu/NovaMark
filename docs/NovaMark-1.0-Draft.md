...
| 迭代 | v0.2-patch2 | 块级 choice、SFX、存档增强、平台模板 | 已完成 |
| Release Candidate | **v1.0.0-rc.1** | 当前基线稳定化、状态契约修订、文档冻结 | ✅ 已完成 |
| Production | **v1.0.0** | 注册重载系统实现、性能基准、完整 API 覆盖 | ✅ 核心功能完成 |
...
- [x] 注册重载系统实现（§4）
- [x] NovaState v2 契约实现（§3.1）
- [x] 状态快照 extensions 字段支持（§4.4）
- [x] stateVersion 兼容机制实现（§4.4.2）
- [x] 注册重载 C API / WASM / JS API 暴露（§4.6, §4.7）
- [ ] 性能基准测试建立
- [x] 完整 API Reference 覆盖
- [x] 注册重载功能测试覆盖（333 tests）
