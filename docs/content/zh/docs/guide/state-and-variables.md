...
`@set` 支持表达式，而且现在也支持 **setOrCreate** 语义：如果左侧变量还不存在，就会在当前作用域里自动创建。
...
不过要注意：**右侧表达式**里读取到的变量，仍然必须已经存在。

```nvm
@set hp = gold + 20 // 如果 gold 还没定义，这里仍然会报错
```

如果你希望更明确地表达“这是一个预先声明的状态”，仍然可以继续使用 `@var`。
